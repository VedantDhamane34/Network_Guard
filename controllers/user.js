...
async function handleUserSignup(req, res) {
    ...
    console.log(recaptchaSignupResponse);
    if(!recaptchaSignupResponse)
    {
        console.log("Not Genearated bhai!!!");
        return res.redirect('/auth');
    }
...
function normalizeIPAddress(ip) {
    ...
    if (ip === '::1') {
        console.log("local jj");
        return '127.0.0.1';
    }

    // If IP is IPv6, convert to IPv4 format
    if (ip.includes(':')) {
        console.log("local nhi re");
        const parts = ip.split(':');
        return parts[parts.length - 1];
    }

    console.log("atta bagh kjwqh",ip);
