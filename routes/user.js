...
// routes/user.js
// In your routes file (e.g., user.js)


// In your routes file (user.js)

router.get('/analyze-packet', async (req, res) => {
    try {
        const packets = await Packet.find({ attack: { $ne: 'normal.' } }); // Exclude normal attacks
        res.render('analyze-packet', { packets }); // Pass packets to the EJS template
    } catch (error) {
        console.error(error);
        res.status(500).send("Server Error");
    }
